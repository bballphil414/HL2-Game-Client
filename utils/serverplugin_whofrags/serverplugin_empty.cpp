...
//---------------------------------------------------------------------------------
class WhoFragsServerPlugin: public IServerPluginCallbacks, public IGameEventListener
{
public:
	WhoFragsServerPlugin();
	~WhoFragsServerPlugin();
...
//
WhoFragsServerPlugin g_EmtpyServerPlugin;
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(WhoFragsServerPlugin, IServerPluginCallbacks, INTERFACEVERSION_ISERVERPLUGINCALLBACKS, g_EmtpyServerPlugin );

//---------------------------------------------------------------------------------
// Purpose: constructor/destructor
//---------------------------------------------------------------------------------
WhoFragsServerPlugin::WhoFragsServerPlugin()
{
...
//---------------------------------------------------------------------------------
bool WhoFragsServerPlugin::Load(	CreateInterfaceFn interfaceFactory, CreateInterfaceFn gameServerFactory )
{
...
//---------------------------------------------------------------------------------
void WhoFragsServerPlugin::Unload( void )
{
...
//---------------------------------------------------------------------------------
void WhoFragsServerPlugin::Pause( void )
{
}
...
//---------------------------------------------------------------------------------
void WhoFragsServerPlugin::UnPause( void )
{
}
...
//---------------------------------------------------------------------------------
const char *WhoFragsServerPlugin::GetPluginDescription( void )
{
...
//---------------------------------------------------------------------------------
void WhoFragsServerPlugin::LevelInit( char const *pMapName )
{
...
//---------------------------------------------------------------------------------
void WhoFragsServerPlugin::ServerActivate( edict_t *pEdictList, int edictCount, int clientMax )
{
}
...
//---------------------------------------------------------------------------------
void WhoFragsServerPlugin::GameFrame( bool simulating )
{
	if ( simulating )
	{
		Bot_RunAll();
...
//---------------------------------------------------------------------------------
void WhoFragsServerPlugin::LevelShutdown( void ) // !!!!this can get called multiple times per map change
{
...
//---------------------------------------------------------------------------------
void WhoFragsServerPlugin::ClientActive( edict_t *pEntity )
{
}
...
//---------------------------------------------------------------------------------
void WhoFragsServerPlugin::ClientDisconnect( edict_t *pEntity )
{
}
...
// Purpose: called on 
//---------------------------------------------------------------------------------
void WhoFragsServerPlugin::ClientPutInServer( edict_t *pEntity, char const *playername )
{
...
// Purpose: called on level start
//---------------------------------------------------------------------------------
void WhoFragsServerPlugin::SetCommandClient( int index )
{
...
// Purpose: called on level start
//---------------------------------------------------------------------------------
void WhoFragsServerPlugin::ClientSettingsChanged( edict_t *pEdict )
{
	if ( playerinfomanager )
	{
...
//---------------------------------------------------------------------------------
PLUGIN_RESULT WhoFragsServerPlugin::ClientConnect( bool *bAllowConnect, edict_t *pEntity, const char *pszName, const char *pszAddress, char *reject, int maxrejectlen )
{
...
//---------------------------------------------------------------------------------
PLUGIN_RESULT WhoFragsServerPlugin::ClientCommand( edict_t *pEntity )
{
	...
	else if ( FStrEq( pcmd, "wf_register" ) )
	{
		KeyValues *kv = new KeyValues( "entry" );
		kv->SetString( "title", "WhoFrags Registration" );
		kv->SetString( "msg", "Enter your E-Mail address to tie your SteamID to your WhoFrags account." );
...
//---------------------------------------------------------------------------------
PLUGIN_RESULT WhoFragsServerPlugin::NetworkIDValidated( const char *pszUserName, const char *pszNetworkID )
{
	return PLUGIN_CONTINUE;
}
...
int getIndexFromUserID(int userid)
{
	...
	for(int i = 1; i <= 64; i++) //TODO: Change 64 to 'maxplayers' max number of players allowed on the server.
	{
...
//---------------------------------------------------------------------------------
void WhoFragsServerPlugin::FireGameEvent( KeyValues * event )
{

	const char * name = event->GetName();
	Msg( "WhoFragsServerPlugin::FireGameEvent: Got event \"%s\"\n", name );

	//Write the event to the wflog.txt file.
	ofstream myfile;
	myfile.open ("wflog.txt",ios::app); //Open the file to append data to it.

	//From AlliedModders Forum
	//const char * name = event->GetName();
	Msg( "DEBUG: WhoFrags::FireGameEvent: Got event \"%s\"\n",name );
	myfile << "<wfevent name=\""<<name<<"\">\n";
	Msg( "( " );
	//myfile << "< ";
	for( KeyValues *pKey = event->GetFirstSubKey(); pKey; pKey = pKey->GetNextKey() )
	{
		//If this event has a userid, add the steamid of the user to the log.
		//int nameDiff;
		//nameDiff = string.compare("userid",pKey->GetName());
		//string paramName = pKey->GetName();
		//string userId = "userid";
		//char * s2 = pKey->GetName();
		if(!strcmp("userid",pKey->GetName()))
		{
			Msg("USER ID LOGGED - TRANSLATES TO INDEX UNKNOWN");
		...
		case TYPE_STRING:
			...
			myfile << "\t<" << pKey->GetName() << "><![CDATA[" << pKey->GetString() << "]]></" << pKey->GetName() << ">\n";
			break;
		case TYPE_INT:
			Msg( "\"%s\" %d; ", pKey->GetName(), pKey->GetInt() );
			//myfile << "\"" << pKey->GetName() << "\" \"" << pKey->GetInt() << "\"; ";
			myfile << "\t<" << pKey->GetName() << ">" << pKey->GetInt() << "</" << pKey->GetName() << ">\n";
			break;
		case TYPE_FLOAT:
			Msg( "\"%s\" %f; ", pKey->GetName(), pKey->GetFloat() );
			//myfile << "\"" << pKey->GetName() << "\" \"" << pKey->GetFloat() << "\"; ";
			myfile << "\t<" << pKey->GetName() << ">" << pKey->GetFloat() << "</" << pKey->GetName() << ">\n";
			break;
		default:
			Msg( "\"%s\" <#%d>; ", pKey->GetName(), pKey->GetDataType() );
			//myfile << "\"" << pKey->GetName() << "\" \"" << pKey->GetDataType() << "\"; ";
			myfile << "\t<" << pKey->GetName() << ">" << pKey->GetDataType() << "</" << pKey->GetName() << ">\n";
			break;
		}
	}
	Msg( ")\n" );
	...
	//Logic to send event to server, or add event to buffer.
...
//---------------------------------------------------------------------------------
CON_COMMAND( wf_version, "prints the version of the empty plugin" )
{
	Msg( "Version:1.0.0.0\n" );
}
